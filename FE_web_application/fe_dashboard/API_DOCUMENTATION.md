...
**Lưu ý:** Base URL có thể được cấu hình thông qua biến môi trường `REACT_APP_API_URL`. Mặc định là `http://159.223.61.25:3000` nếu không có biến môi trường.
...
- `page`: Số trang (default: 1, phải >= 1)
- `limit`: Số lượng mỗi trang (default: 10, phải > 0)
- `search`: Tìm kiếm (string)
- `sortBy`: Sắp xếp theo field (string)
- `sortOrder`: `asc` hoặc `desc` (string)
- `userId`: ID của user (chỉ Admin có thể dùng)

**Lưu ý:** 
- Nếu không truyền `page`, mặc định sẽ là `page=1`
- Nếu `page` không hợp lệ (< 1 hoặc không phải số), sẽ tự động set về `1`
- Các parameters khác là optional và chỉ được gửi nếu có giá trị hợp lệ

**cURL:**
```bash
# Không truyền parameters (sẽ dùng default)
curl -X GET "http://localhost:3000/garden" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN"

# Truyền đầy đủ parameters
curl -X GET "http://localhost:3000/garden?page=1&limit=10&search=vườn&sortBy=name&sortOrder=asc" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN"
```
...
### 3.6. Lấy danh sách tất cả vườn (Admin only)

**Endpoint:** `GET /garden/admin`

**Headers:** `Authorization: Bearer YOUR_ACCESS_TOKEN`

**cURL:**
```bash
curl -X GET http://localhost:3000/garden/admin \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN"
```

---

### 3.7. Lấy thông tin vườn theo ID (Admin only)

**Endpoint:** `GET /garden/admin/:id`

**Headers:** `Authorization: Bearer YOUR_ACCESS_TOKEN`

**cURL:**
```bash
curl -X GET http://localhost:3000/garden/admin/1 \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN"
```

---

### 3.8. Tạo vườn mới (Admin only)

**Endpoint:** `POST /garden/admin`
...
**cURL:**
```bash
curl -X POST http://localhost:3000/garden/admin \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "Vườn A",
...
### 3.9. Cập nhật vườn (Admin only)

**Endpoint:** `PUT /garden/admin/:id`
...
**cURL:**
```bash
curl -X PUT http://localhost:3000/garden/admin/1 \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "Vườn A Updated",
...
### 3.10. Xóa vườn (Admin only - Admin endpoint)

**Endpoint:** `DELETE /garden/admin/:id`

**Headers:** `Authorization: Bearer YOUR_ACCESS_TOKEN`

**cURL:**
```bash
curl -X DELETE http://localhost:3000/garden/admin/1 \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN"
```
...
# Truyền đầy đủ parameters
curl -X GET "http://localhost:3000/vegetable?page=1&limit=10&search=rau&sortBy=name&sortOrder=asc" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN"
```
...
**Query Parameters:**
- `type`: `day`, `week`, hoặc `month` **(required)**
...
**Lưu ý:** Parameter `type` là **bắt buộc**. Nếu không truyền hoặc truyền giá trị không hợp lệ, API sẽ trả về lỗi 400 Bad Request.
...
**Lưu ý:** API này thực hiện soft delete (đánh dấu xóa) thay vì xóa vĩnh viễn.
