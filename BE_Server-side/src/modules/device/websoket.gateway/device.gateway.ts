...
export class WsGateway implements OnGatewayConnection {
  ...
  async handleSelectGarden(
    ...
    const latestData = await this.deviceService.getLatestGardenData(data.deviceMac);
  ...
  async handleControlPump(
    ...
    try {
      const result = await this.deviceService.controlPump(payload.mac, payload.action);
  ...
  sendSensorDataToRoom(macAddress: string, data: any) {
    this.server.to(macAddress).emit('sensorData', data);
  }
}