...
## 2. Lấy logs của user hiện tại

Lấy danh sách audit logs của chính user đang đăng nhập.
...
```javascript
// services/auditService.js
export const getMyLogs = async (limit = 50, token) => {
  const response = await fetch(`${API_BASE_URL}/audit/my-logs?limit=${limit}`, {
    method: 'GET',
    headers: {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json',
...
function MyActivityLog() {
  ...
  useEffect(() => {
    const fetchLogs = async () => {
      setLoading(true);
      try {
        const data = await getMyLogs(100, token);
        setLogs(data);
      } catch (error) {
        console.error('Error fetching logs:', error);
      } finally {
...
| GET | `/audit/my-logs` | Lấy logs của user hiện tại | USER, ADMIN |
