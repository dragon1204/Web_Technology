...
|------|------|-------|-----------|
| `00` | `SUCCESS` | Thanh toán thành công | Cập nhật order: `paymentStatus = PAID`, `status = CONFIRMED` |
...
### 6. Webhook xử lý (Backend tự động)
- PayOS gọi webhook khi thanh toán thành công
- Backend cập nhật order status
- Frontend có thể kiểm tra status qua polling
...
```javascript
// Trong PaymentSuccessPage
useEffect(() => {
  const orderId = new URLSearchParams(window.location.search).get('orderId');
  
  if (orderId) {
    // Verify payment status
    getPaymentStatus(orderId, token).then((result) => {
